  ...
  cloud_init = base64encode(<<-CLOUD_INIT
    ...
    runcmd:
      ...
      - |
        ...
        #!/bin/bash
        source /etc/profile.d/app_env.sh
        if mysql -h "$MYSQL_READ_REPLICA_HOST" -P "$MYSQL_PORT" -u "$MYSQL_USER" -p "$REPLICA_MYSQL_PASS" "$MYSQL_DB" -e "SELECT 1;" --init-command="SET SESSION wait_timeout=$MYSQL_TIMEOUT" &>/dev/null; then

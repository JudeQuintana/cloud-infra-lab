...
locals {
  ...
  cloud_init = base64encode(<<-CLOUD_INIT
    ...
    runcmd:
      ...
      - echo 'export MYSQL_USER="${local.secretsmanager_mysql.username}"' >> /etc/profile.d/app_env.sh
      - echo 'export MYSQL_PASS="${local.secretsmanager_mysql.password}"' >> /etc/profile.d/app_env.sh
      - echo 'export MYSQL_DB="${local.secretsmanager_mysql.db_name}"' >> /etc/profile.d/app_env.sh
